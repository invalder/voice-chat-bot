...
impl RagSystem {
    ...
    pub fn new(db_path: String, collection_name: String) -> Result<Self> {
        ...
        if !Path::new(&db_path).exists() {
            fs::create_dir_all(&db_path).context("Failed to create database directory")?;
        }
    ...
    fn generate_answer(&self, _query: &str, results: &[QueryResult]) -> String {
        ...
        if results.len() > 1 {
            answer.push_str(&format!(
                "\n\n(Found {} relevant information sources)",
                results.len()
            ));
        }
    ...
    fn save_documents(&self) -> Result<()> {
        ...
        fs::write(&db_file, json).context("Failed to write documents to disk")?;
        Ok(())
    }
    ...
    fn load_documents(&mut self) -> Result<()> {
        ...
        let json = fs::read_to_string(&db_file).context("Failed to read documents from disk")?;

        self.documents = serde_json::from_str(&json).context("Failed to deserialize documents")?;
...
mod tests {
    ...
    fn test_rag_basic() {
        ...
        let mut rag =
            RagSystem::new(temp_dir.to_string_lossy().to_string(), "test".to_string()).unwrap();

        rag.add_document("Rust is a systems programming language.".to_string(), None)
            .unwrap();
