...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::LoadKnowledge { path }) => {
            ...
            println!("Total documents: {}", stats.get("total_documents").unwrap_or(&"0".to_string()));
        }
...
fn display_stats(bot: &VoiceChatBot) {
    let stats = bot.get_stats();
    println!("\nBot Statistics:");
    println!("  Expert Domain: {}", stats.get("expert_domain").unwrap_or(&"unknown".to_string()));
    println!("  STT Model: {}", stats.get("stt_model").unwrap_or(&"unknown".to_string()));
    println!("  TTS Engine: {}", stats.get("tts_engine").unwrap_or(&"unknown".to_string()));
    println!("  Total Documents: {}", stats.get("total_documents").unwrap_or(&"0".to_string()));
}
