...
pub use rag::{RagSystem, QueryResponse};
...
impl VoiceChatBot {
    ...
    pub fn new(config: VoiceChatBotConfig) -> Result<Self> {
        ...
        let stt = SpeechToText::new(
            config.stt.model_name.clone(),
            config.stt.language.clone(),
        )?;
        
        let tts = TextToSpeech::new(
            config.tts.engine.clone(),
            config.tts.rate,
        )?;
        
        let rag = RagSystem::new(
            config.db_path.clone(),
            config.expert_domain.clone(),
        )?;
    ...
    pub fn load_knowledge_base(&mut self, path: Option<&str>) -> Result<()> {
        ...
        info!("Total documents in knowledge base: {}", stats.get("total_documents").unwrap_or(&"0".to_string()));
        
        Ok(())
    }
    
    /// Add knowledge to the bot's knowledge base
    pub fn add_knowledge(&mut self, text: String, metadata: Option<HashMap<String, String>>) -> Result<String> {
    ...
    pub fn process_voice_query(&self, audio_path: Option<&str>, duration: Option<u32>) -> Result<String> {
    ...
    pub fn get_stats(&self) -> HashMap<String, String> {
        ...
        stats.insert("expert_domain".to_string(), self.config.expert_domain.clone());
        ...
        stats.insert("embedding_model".to_string(), self.config.rag.embedding_model.clone());
        stats
    }
}
